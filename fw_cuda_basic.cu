...
void initializeArray2D(int *arr1, int *arr2, int N, int seed);
void host_FW(int *d, int N);
void fw_cuda_basic(int *d_d, int N);
...
int main (int argc, char **argv) {
        int N = ARR_DIM;    // Size of the distance matrix
        ...
        dim3 dimGrid( (N + BLOCK_DIM - 1) / BLOCK_DIM, 
                      (N + BLOCK_DIM - 1) / BLOCK_DIM );       // Grid size
                ...
                for (int j = 0; j < N; j++) {
                        ...
                        if (i == j) {
                                ...
                                if (randNum < 0.35) {
                                        ...
                                        arr1[IDX(i, j, N)] = rand() % 50 + 50;    // Random value between 50 and 99
                                        arr2[IDX(i, j, N)] = arr1[IDX(i, j, N)];
                                }
                        }
